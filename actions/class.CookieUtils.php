...
class taoLti_actions_CookieUtils extends tao_actions_CommonModule
{
    ...
    */
    public function restoreSession() {
        $sessId = HeaderParser::getSessionFromCookie($this->getHeader('Cookie'));
        $url = $this->getRequestParameter('redirect');
        ...
        if (session_id() != $sessId) {
            ...
            common_Logger::d('regenerated session to id \'' . substr(session_id(), 0, 7) . '...\'');
        } else {
            common_Logger::w('Restore session called with correct session id \'' . substring(session_id(), 0, 7) . '...\'');
        }
        $this->forwardUrl($url);
    }
}